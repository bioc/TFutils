...
reference data, tools for visualizing
...
```{r lktarapp, fig.cap='TFtargs() screenshot.',echo=FALSE}
...
**TFs that are direct GWAS hits.** The following function accepts
...
directHitsInCISBP = function(traitTag, gwascat) {
   ...
   as.data.frame(mcols(gwascat)) %>%  # names are normalized
     dplyr::select(DISEASE.TRAIT, MAPPED_GENE) %>% 
     dplyr::filter(DISEASE.TRAIT==traitTag) %>% 
     mutate(HGNC=MAPPED_GENE) %>% 
     inner_join(cisbpTFcat) %>% 
